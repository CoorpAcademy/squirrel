...
export const syncDirectory$ = (client, pathFS, pathETCD) => {
  ...
  return get$(client, pathETCD)
    .catch(err => {
      if (err.errorCode === 100) return mkdir$(client, pathETCD);
      throw err;
    })
    .flatMap(action => {
      if (action && !get('node.dir', action))
        return del$(client, pathETCD).flatMap(() => mkdir$(client, pathETCD));
...
export const syncFile$ = (client, pathFS, pathETCD) => {
  ...
  return get$(client, pathETCD)
    .catch(err => {
      if (err.errorCode === 100) return Observable.of(null);
      throw err;
    })
    .flatMap(action => {
...
export const sync$ = (client, pathFS, pathETCD) => {
  ...
  return isDirectory$(pathFS)
    .flatMap(isDirectory => {
      if (isDirectory) return syncDirectory$(client, pathFS, pathETCD);
      return syncFile$(client, pathFS, pathETCD);
    })
    .flatMap(entry => sync$(client, join(pathFS, entry), join(pathETCD, entry)));
};
