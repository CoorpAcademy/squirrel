...
export const delRecursive$ = (client, key) =>
  debug('delRecursive', key) || del$(client, key, {recursive: true});

export const get$ = wrap('get');

export const getRecursive$ = (client, key) =>
  debug('getRecursive', key) || get$(client, key, {recursive: true});
...
export const rmdirRecursive$ = (client, key) =>
  debug('rmdirRecursive', key) || rmdir$(client, key, {recursive: true});

export const set$ = wrap('set');

export const isDirectory = node => debug('isDirectory', node) || !!(node && node.dir);

export const isFile = node => debug('isFile', node) || !!(node && !node.dir);
